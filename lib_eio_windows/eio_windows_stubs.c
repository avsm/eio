...
CAMLprim value caml_eio_windows_renameat(value v_old_fd, value v_old_path, value v_new_fd, value v_new_path)
{
  ...
  old_dir = Is_some(v_old_fd) ? Handle_val(Field(v_old_fd, 0)) : NULL;
  new_dir = Is_some(v_new_fd) ? Handle_val(Field(v_new_fd, 0)) : NULL;

  // Open the source itself, not a symlink's target, with the right to move it
  ...
  // The new name is relative to [new_dir], or an NT path if there is none
  new_path = caml_stat_strdup_to_utf16(String_val(v_new_path));
  name_len = wcslen(new_path) * sizeof(wchar_t);
  info_size = sizeof(FILE_RENAME_INFORMATION) + name_len;
