...
(* Stress the spawn machinery: many concurrent children, each writing a
   unique token back through its own inherited pipe. A handle leaking into
   a sibling would delay the pipe's EOF; a wrong token or status would
...
let test_stress env () =
  ...
  in
  Fiber.List.iter ~max_fibers:8 spawn_one (List.init 10_000 Fun.id)
...
(* An explicit executable heads the command line: it replaces argv[0]
   and is resolved by CreateProcess's own search. *)
...
(* Windows has no signals: any signal terminates the process, which then
   reports having been killed by that signal. *)
...
(* Waiting for a process can be cancelled, leaving it running. *)
...
(* A working directory outside the sandbox is refused. *)
