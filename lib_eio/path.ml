...
let ( / ) (d, p1) p2 =
  let (Resource.T (dir, ops)) = d in
  let module X = (val (Resource.get ops Fs.Pi.Dir)) in
  if p2 <> "" && not (X.is_step dir p2) then (d, p2)
  else if p1 = "." && p2 <> "" then (d, p2)
  else (d, X.join dir p1 p2)
...
let split (d, p) =
  let (Resource.T (dir, ops)) = d in
  let module X = (val (Resource.get ops Fs.Pi.Dir)) in
  X.split dir p |> Option.map (fun (dirname, basename) -> ((d, dirname), basename))
